...
export function useMappingSession(session: MappingSession) {
  ...
  return useMemo(
    () => ({
      ...
      previewStale: session.getPreviewStale(),
      previewResult: session.getPreviewResult(),
      previewRowCount: session.getPreviewRowCount(),
      jobProgress: session.getJobProgress(),
      inputFile: session.getInputFile(),
      dataRowCount: session.getDataRowCount(),
      columnCount: session.getColumnCount(),
      detectedEncoding: session.getDetectedEncoding(),
      inputSamples: session.getInputSamples(),
    }),
    [session, revision],
  );
}
